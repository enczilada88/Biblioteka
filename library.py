...
class LibraryAll:
    ...
    def get_movies(self):
        movies_only = [item for item in self.library_database if (isinstance(item, Movie) and not isinstance(item, Series))]
        return sorted(movies_only, key=lambda movie: movie.title)

    def get_series(self):
        series_only = [item for item in self.library_database if isinstance(item, Series)]
        return sorted(series_only, key=lambda series: series.title)
    ...
    def top_titles(self, content_type=None, top_counter=3):
        ...
        if content_type is None:
            ...
            movies_only = [item for item in self.library_database if (isinstance(item, Movie) and not isinstance(item, Series))]
            return sorted(movies_only, key=lambda movie: movie.views, reverse=True)[0:top_counter]
        elif content_type == "Series":
            series_only = [item for item in self.library_database if isinstance(item, Series)]
            return sorted(series_only, key=lambda movie: movie.views, reverse=True)[0:top_counter]
        else:
            return top_titles
...
# top titles
    ...
    for item in libAll.top_titles(content_type="Series", top_counter=3):
        print(f"{item.title}: {item.views}")
    print('-' * 100)
# check of contant_type != series/!=movies
    print(f"The most popular series of {date.today()}:\n")
    for item in libAll.top_titles(content_type="check", top_counter=3):
        print(f"{item.title}: {item.views}")
    print('-' * 100)
